 ...
 */
@Controller
public class CheckStoreByNameAction extends ActionSupport implements SessionAware, ServletResponseAware, ServletRequestAware{
    ...
    public String execute(){
        List<StoreAmountEntity> list = new ArrayList<StoreAmountEntity>();
        List<ProductEntity> products = productService.findByName(name);
        if(!products.isEmpty() || products != null){
            for(ProductEntity product : products){
                Integer proId = product.getProductId();
                List<StoreAmountEntity> list1 = storeAmountService.findStoreAmountByProductId(proId);
                list.addAll(list1);
            }
        }
